	...
	public PagingUtil(String keyfield, String keyword, int currentPage, int totalCount, int rowCount,
		...
		if (currentPage > pageCount) {
			if(keyword==null){//검색 미사용시
				...
				pagingHtml.append("<a href="+pageUrl+"?keyfield="+keyfield+"&keyword="+keyword+"&pageNum="+ (startPage - 1) + "&career_year="+ career_year + "&rowCount=" + rowCount +">");
			}
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == currentPage) {
				...
				pagingHtml.append(i);
				pagingHtml.append("&career_year="+ career_year+ "&rowCount=" + rowCount+"'>");
				pagingHtml.append(i);
				pagingHtml.append("</a>");
			}
			pagingHtml.append("&nbsp;");
		...
		if (totalPage - startPage >= pageCount) {
			if(keyword==null){//검색 미사용시
				...
				pagingHtml.append("<a href="+pageUrl+"?keyfield="+keyfield+"&keyword="+keyword+"&pageNum="+ (endPage + 1) + "&career_year="+ career_year+"&rowCount=" + rowCount+">");
			}
